...
def process_data_from_test_csvs(
    ...
    """
    # Hack to use the data in the database, while we pipe things together
    if spacecraft == "SO" and measurement.startswith("B_"):
        return get_so_gse_magnetic_field(measurement, range_param)
...
def get_so_gse_magnetic_field(
    ...
    # Get the relevant data from the DB
    data = pd.DataFrame(
        models.SOGSEMagneticField.objects.filter(time__gte=from_date)
        .order_by("time")
        .values("time", measurement)
    )
